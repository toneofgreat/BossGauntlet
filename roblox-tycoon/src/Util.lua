-- Util.lua -- shared helper API for Boss Tycoon (contract: Util API section)
...
-- opts: {offsetY, width, height, textColor, alwaysOnTop, maxDistance, textSizeScaled}
...
--------------------------------------------------------------------
-- Money formatting: "$999", "$1.5K", "$25M", "$1B", "$2T"
--------------------------------------------------------------------
...
function M.fmt(n)
	...
	if div == 1 then
		...
		local v = math.floor((a / div) * 10) / 10 -- one decimal max
		if v >= 100 or v == math.floor(v) then
			s = tostring(math.floor(v)) -- trims ".0"
		else
