...
export function colliderDescFromShape(shape, size, position, rotationDeg, opts = {}) {
  ...
  // The 2026-09-25 shape pack (spec 03 §5.2a/§5.4a). There are five analytic collider
  // kinds and nine new shapes, so each new shape collides as the SMALLEST supported
  // volume that CONTAINS it — never a smaller one. Containing is the safe direction: a
  // player may stand on air at the tip of a cone (its collider is the cylinder around
  // it) but can never fall through a solid, walk inside a wall, or be killed by a kill
  // brick they are visibly clear of. The pairings are in §5.4a's table; the round ones
  // take the cylinder branch below and the rest fall through to box.
  if (shape === "cone" || shape === "prism" || shape === "dome" || shape === "capsule" || shape === "tube") {
    shape = "cylinder";
  }
  if (shape === "cylinder") {
