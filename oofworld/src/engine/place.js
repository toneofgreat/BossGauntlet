...
const LIGHT_DEFAULTS = Object.freeze({ intensity: 1.2, range: 24 });
...
export function applyPartDefaults(def, index) {
  ...
  const full = {
    ...
    // A lamp defaults to the part's own colour at 1.2 brightness over 24 studs, so
    // `light: {}` on a neon sphere is already the lamp anyone meant by it.
    light: isPlainObject(src.light)
      ? {
        color: src.light.color || src.color || PART_DEFAULTS.color,
        intensity: src.light.intensity != null ? src.light.intensity : LIGHT_DEFAULTS.intensity,
        range: src.light.range != null ? src.light.range : LIGHT_DEFAULTS.range,
      }
      : null,
    behaviors,
  };
...
// APPEND-ONLY, both of them: pack.js encodes a shape and a material as their INDEX in
// this exact order (spec 11 §3.4), so inserting or reordering a name silently
// reinterprets every share code ever published. The nine shapes and eight materials
// after the originals are the 2026-09-25 pack (spec 03 §5.1a/§5.2a).
...
// A lamp block (spec 04 §3.1, added 2026-09-25). `light` is absent on almost every part;
// where it is present the part asks the renderer's pool for a point light at its centre.
const LIGHT_ALLOWED = new Set(["color", "intensity", "range"]);
const LIGHT_MAX_INTENSITY = 8;
const LIGHT_MAX_RANGE = 200;

function validateLight(errors, path, light) {
  if (!isPlainObject(light)) { pushErr(errors, path, "expected an object"); return; }
  checkAdditional(errors, path, light, LIGHT_ALLOWED);
  if ("color" in light) checkPattern(errors, `${path}.color`, light.color, HEX_RE, "^#[0-9a-f]{6}$");
  if ("intensity" in light) checkNumber(errors, `${path}.intensity`, light.intensity, 0, LIGHT_MAX_INTENSITY);
  if ("range" in light) checkNumber(errors, `${path}.range`, light.range, 1, LIGHT_MAX_RANGE);
}
...
function validatePart(errors, path, part) {
  ...
  if ("light" in part) validateLight(errors, `${path}.light`, part.light);
...
function pointInsidePart(point, position, size, shape) {
  ...
  // Every shape whose collider IS the analytic cylinder (spec 03 §5.4a) — so this static
  // check agrees with the collider the player will actually hit, rather than testing a
  // cone against the box around it.
  if (shape === "cylinder" || shape === "cone" || shape === "prism"
    || shape === "dome" || shape === "capsule" || shape === "tube") {
