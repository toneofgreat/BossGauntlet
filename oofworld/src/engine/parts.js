...
// ---- lamps: parts carrying a `light` block (spec 03 §5.1b, spec 04 §3.1) -----------
// A Place does not add lights; it marks parts as lamps and the renderer lends its pool
// out to the nearest ones (spec 02 §5.2a). This module keeps the ids and hands the
// renderer a PULL function, so a lamp bolted to a movingPlatform or a spinner is lit
// wherever the part currently is without any per-step bookkeeping.
// id -> the lamp's CURRENT { color, intensity, range }. Deliberately not `def.light`:
// place.js deep-freezes every def it loads (spec 04 §3.1's "defaults applied exactly
// once"), so a Place dimming its own lamps at dusk could not write there, and a Place
// dimming its lamps at dusk is the whole reason setLight exists.
const lampLights = new Map();
let lightsRef = null;

const LIGHT_DEFAULT_INTENSITY = 1.2;
const LIGHT_DEFAULT_RANGE = 24;

// Defaulting lives here as well as in place.js because addPart/create/createMany take
// raw defs at runtime (see normalizeRuntimePart's banner).
function normalizeLight(src, partColor) {
  if (!src || typeof src !== "object") return null;
  return {
    color: typeof src.color === "string" ? src.color : partColor,
    intensity: typeof src.intensity === "number" ? src.intensity : LIGHT_DEFAULT_INTENSITY,
    range: typeof src.range === "number" ? src.range : LIGHT_DEFAULT_RANGE,
  };
}

function trackLamp(def) {
  if (def && def.light) lampLights.set(def.id, { ...def.light });
}

function collectLamps() {
  const out = [];
  for (const [id, light] of lampLights) {
    const record = partsById.get(id);
    if (!record) continue;
    // An instanced part shares one InstancedMesh with its whole batch, so its own
    // position only exists on the def — which is fine, because instanced parts are
    // anchored and immutable by construction (spec 03 §5.3).
    const p = record.instanced ? record.def.position : record.mesh.position;
    out.push({
      position: record.instanced ? [p[0], p[1], p[2]] : [p.x, p.y, p.z],
      color: light.color,
...
function normalizeRuntimePart(def) {
  ...
  return {
    ...
    light: normalizeLight(src.light, src.color || "#a3a2a5"),
...
export function init(deps) {
  ...
  // Optional so every existing caller and test that inits parts without a renderer
  // (Node-side rule checks, fixtures) keeps working — with no pool, lamps are just
  // parts that glow by their material.
  lightsRef = deps.lights || null;
  if (lightsRef) lightsRef.setProvider(collectLamps);
}
...
export function setEmissiveIntensity(id, mult) {
  ...
  if (EMISSIVE_BASE[mat] === undefined) {
  ...
  const base = EMISSIVE_BASE[mat]; // spec 03 §5.1 recipe table
  record.mesh.material.emissiveIntensity = base * mult;
}

// setLight(id, light | null) — turn a part into a lamp, retune it, or stop it being one
// (spec 03 §5.1b). Unlike the other setters this one works on an INSTANCED part too: a
// lamp is not the mesh, so nothing about the shared InstancedMesh is being mutated.
export function setLight(id, light) {
  const record = partsById.get(id);
  if (!record) throw new Error(`part ${id} not found`);
  const next = normalizeLight(light, record.def.color);
  if (next) lampLights.set(id, next);
  else lampLights.delete(id);
}
...
// ---- the 2026-09-25 material pack (spec 03 §5.1a) ----------------------------------
// Eight more recipes in the same contract as the six above: fill a TEX_SIZE² luminance
// buffer from the shared mulberry32(fnv1a(name)) stream, so a name always produces the
// same pixels on every device and no two players see a different world. `galaxy` is the
// only one that also lights itself (see buildMaterial) — everything else is a map.

function buildBrickL(rng) {
  const L = new Float64Array(TEX_SIZE * TEX_SIZE).fill(150); // mortar
  const ROWS = 8;
  const rowH = TEX_SIZE / ROWS; // 16
  const BRICK_W = 32;
  const MORTAR = 2;
  for (let r = 0; r < ROWS; r++) {
    const offset = r % 2 === 0 ? 0 : BRICK_W / 2; // running bond
    for (let b = -1; b < TEX_SIZE / BRICK_W + 1; b++) {
      const x0 = b * BRICK_W + offset + MORTAR;
      const shade = 205 + rr(rng, -22, 14);
      for (let y = Math.round(r * rowH) + MORTAR; y < Math.round((r + 1) * rowH); y++) {
        for (let x = x0; x < x0 + BRICK_W - MORTAR; x++) {
          if (x < 0 || x >= TEX_SIZE) continue;
          // A brick is not flat: a little grain plus a lit top-left bevel.
          const bevel = Math.min(3, Math.min(x - x0, y - (r * rowH + MORTAR))) / 3;
          L[y * TEX_SIZE + x] = shade + rr(rng, -6, 6) + 14 * (1 - bevel);
        }
      }
    }
  }
...
function buildMarbleL(rng) {
  const L = new Float64Array(TEX_SIZE * TEX_SIZE);
  const warp = buildLattice2D(rng, 8);
  const grain = buildLattice2D(rng, 32);
  for (let y = 0; y < TEX_SIZE; y++) {
    for (let x = 0; x < TEX_SIZE; x++) {
      const u = (x / TEX_SIZE) * 8, v = (y / TEX_SIZE) * 8;
      // Veins are a sine of a warped coordinate — the standard marble trick: the warp
      // is what keeps them from reading as wallpaper stripes.
      const w = sampleValueNoise2D(warp, u, v);
      const vein = Math.abs(Math.sin((x + y) * 0.06 + w * 7));
      const fine = sampleValueNoise2D(grain, (x / TEX_SIZE) * 32, (y / TEX_SIZE) * 32);
      L[y * TEX_SIZE + x] = 246 - 70 * (1 - vein) ** 3 - 8 * fine;
    }
  }
  return L;
}

function buildSandL(rng) {
  const L = new Float64Array(TEX_SIZE * TEX_SIZE);
  const dune = buildLattice2D(rng, 6);
  for (let y = 0; y < TEX_SIZE; y++) {
    for (let x = 0; x < TEX_SIZE; x++) {
      const ripple = Math.sin(y / 5 + sampleValueNoise2D(dune, (x / TEX_SIZE) * 6, (y / TEX_SIZE) * 6) * 6);
      L[y * TEX_SIZE + x] = 226 + 9 * ripple + rr(rng, -14, 14);
    }
  }
  const specks = Math.round(TEX_SIZE * TEX_SIZE * 0.004);
  for (let i = 0; i < specks; i++) L[ri(rng, 0, TEX_SIZE - 1) * TEX_SIZE + ri(rng, 0, TEX_SIZE - 1)] = 255;
  return L;
}

function buildConcreteL(rng) {
  const L = new Float64Array(TEX_SIZE * TEX_SIZE);
  const a = buildLattice2D(rng, 10);
  const b = buildLattice2D(rng, 40);
  for (let y = 0; y < TEX_SIZE; y++) {
    for (let x = 0; x < TEX_SIZE; x++) {
      const n = 0.7 * sampleValueNoise2D(a, (x / TEX_SIZE) * 10, (y / TEX_SIZE) * 10)
        + 0.3 * sampleValueNoise2D(b, (x / TEX_SIZE) * 40, (y / TEX_SIZE) * 40);
      L[y * TEX_SIZE + x] = 196 + 26 * n + rr(rng, -5, 5);
    }
  }
  for (let p = 0; p < 30; p++) { // air pockets
    const cx = ri(rng, 0, TEX_SIZE - 1), cy = ri(rng, 0, TEX_SIZE - 1), r = rr(rng, 1.5, 4);
    for (let y = Math.floor(cy - r); y <= cy + r; y++) {
      for (let x = Math.floor(cx - r); x <= cx + r; x++) {
        if (x < 0 || y < 0 || x >= TEX_SIZE || y >= TEX_SIZE) continue;
        const d = Math.hypot(x - cx, y - cy);
        if (d <= r) L[y * TEX_SIZE + x] -= 45 * (1 - d / r);
      }
    }
  }
  return L;
...
function buildGoldL(rng) {
  const L = new Float64Array(TEX_SIZE * TEX_SIZE);
  const lattice = buildLattice1D(rng, Math.ceil((TEX_SIZE - 1) / 9) + 2);
  for (let y = 0; y < TEX_SIZE; y++) {
    const base = 224 + 18 * sampleValueNoise1D(lattice, y / 9);
    for (let x = 0; x < TEX_SIZE; x++) {
      // Two sheen bands rather than metal's one, tighter and brighter: polished, not
      // brushed. The map is greyscale — the gold is whatever colour the part carries.
      const d1 = Math.abs(x + y - 70) / Math.SQRT2;
      const d2 = Math.abs(x + y - 190) / Math.SQRT2;
      L[y * TEX_SIZE + x] = base + rr(rng, -4, 4)
        + 30 * Math.exp(-((d1 / 11) ** 2)) + 22 * Math.exp(-((d2 / 14) ** 2));
    }
  }
  for (let s = 0; s < 40; s++) { // hairline scratches
    const y0 = ri(rng, 0, TEX_SIZE - 1), len = ri(rng, 8, 40), x0 = ri(rng, 0, TEX_SIZE - 1);
    for (let i = 0; i < len; i++) {
      const x = (x0 + i) % TEX_SIZE;
      L[y0 * TEX_SIZE + x] -= 16;
    }
  }
  return L;
...
function buildCobbleL(rng) {
  // Voronoi cells from a jittered 6x6 lattice: stones with dark gaps between them and
  // a highlight at each stone's middle, which is what makes them read as domed.
  const CELLS = 6;
  const step = TEX_SIZE / CELLS;
  const sites = [];
  for (let cy = 0; cy < CELLS; cy++) {
    for (let cx = 0; cx < CELLS; cx++) {
      sites.push([(cx + rr(rng, 0.2, 0.8)) * step, (cy + rr(rng, 0.2, 0.8)) * step, 198 + rr(rng, -26, 26)]);
    }
  }
  const L = new Float64Array(TEX_SIZE * TEX_SIZE);
  for (let y = 0; y < TEX_SIZE; y++) {
    for (let x = 0; x < TEX_SIZE; x++) {
      let d0 = Infinity, d1 = Infinity, shade = 200;
      for (const s of sites) {
        // Wrap in both axes so the texture tiles without a visible seam.
        let dx = Math.abs(x - s[0]); if (dx > TEX_SIZE / 2) dx = TEX_SIZE - dx;
        let dy = Math.abs(y - s[1]); if (dy > TEX_SIZE / 2) dy = TEX_SIZE - dy;
        const d = Math.hypot(dx, dy);
        if (d < d0) { d1 = d0; d0 = d; shade = s[2]; } else if (d < d1) { d1 = d; }
      }
      const edge = smoothstep(clamp((d1 - d0) / 5, 0, 1)); // 0 at a gap, 1 mid-stone
      L[y * TEX_SIZE + x] = 120 + (shade - 120) * edge + 16 * edge * edge + rr(rng, -4, 4);
    }
  }
  return L;
}

function buildSnowL(rng) {
  const L = new Float64Array(TEX_SIZE * TEX_SIZE);
  const lumps = buildLattice2D(rng, 12);
  for (let y = 0; y < TEX_SIZE; y++) {
    for (let x = 0; x < TEX_SIZE; x++) {
      const n = sampleValueNoise2D(lumps, (x / TEX_SIZE) * 12, (y / TEX_SIZE) * 12);
      L[y * TEX_SIZE + x] = 243 + 12 * n;
    }
  }
  const sparkles = Math.round(TEX_SIZE * TEX_SIZE * 0.006);
  for (let i = 0; i < sparkles; i++) L[ri(rng, 0, TEX_SIZE - 1) * TEX_SIZE + ri(rng, 0, TEX_SIZE - 1)] = 255;
  return L;
}

function buildGalaxyL(rng) {
  // Dark sky, one nebula band across the diagonal, and stars of three sizes. This map
  // is used as BOTH the colour map and the emissive map (buildMaterial), so the bright
  // pixels are the only part that glows: a starfield you can read in a night Place.
  const L = new Float64Array(TEX_SIZE * TEX_SIZE);
  const neb = buildLattice2D(rng, 5);
  for (let y = 0; y < TEX_SIZE; y++) {
    for (let x = 0; x < TEX_SIZE; x++) {
      const band = Math.exp(-(((x - y) / 34) ** 2));
      const n = sampleValueNoise2D(neb, (x / TEX_SIZE) * 5, (y / TEX_SIZE) * 5);
      L[y * TEX_SIZE + x] = 18 + 70 * band * n + 10 * n;
    }
  }
  const star = (x, y, v) => {
    if (x < 0 || y < 0 || x >= TEX_SIZE || y >= TEX_SIZE) return;
    L[y * TEX_SIZE + x] = Math.max(L[y * TEX_SIZE + x], v);
  };
  for (let i = 0; i < 260; i++) star(ri(rng, 0, TEX_SIZE - 1), ri(rng, 0, TEX_SIZE - 1), rr(rng, 120, 200));
  for (let i = 0; i < 40; i++) {
    const x = ri(rng, 1, TEX_SIZE - 2), y = ri(rng, 1, TEX_SIZE - 2);
    star(x, y, 255); star(x + 1, y, 190); star(x - 1, y, 190); star(x, y + 1, 190); star(x, y - 1, 190);
  }
  return L;
}
...
function buildLuminanceBuffer(name, rng) {
  switch (name) {
    ...
    case "brick": return buildBrickL(rng);
    case "marble": return buildMarbleL(rng);
    case "sand": return buildSandL(rng);
    case "concrete": return buildConcreteL(rng);
    case "gold": return buildGoldL(rng);
    case "cobble": return buildCobbleL(rng);
    case "snow": return buildSnowL(rng);
    case "galaxy": return buildGalaxyL(rng);
    default:
...
// Which materials have an emissive channel, and the base intensity setEmissiveIntensity
// multiplies. Single source for both the builder below and the public setter above —
// they disagreed about lava's 0.85 in two places before this table existed.
const EMISSIVE_BASE = Object.freeze({ neon: 1.0, lava: 0.85, galaxy: 0.55 });
...
function buildMaterial(name, colorHex, tBucket) {
  ...
  if (EMISSIVE_BASE[name] !== undefined) {
    // Lava (and, since 2026-09-25, galaxy) keep their emissive fields even when also
    // made transparent below. The map doubles as the emissive map, so only the recipe's
    // bright pixels glow — cracks for lava, stars for galaxy.
    fields.emissive = colorHex;
    fields.emissiveMap = map;
    fields.emissiveIntensity = EMISSIVE_BASE[name];
  }
...
// ---- the 2026-09-25 shape pack (spec 03 §5.2a) -------------------------------------
// Nine shapes beyond the original four. Every one is normalised to the UNIT BOX
// (x, y, z each in [-0.5, 0.5]) exactly like box/cylinder/sphere/wedge, because the
// mesh layer scales geometry by the part's `size` and the geometry cache is keyed by
// shape name alone — a shape that did not fill the unit box would silently report a
// size it does not have to every consumer (the Studio gizmo, the collider, the grid
// AABB). Two of them cannot fill all three axes and say so where they are built:
// `ring` and `star` are PLATES, and their thickness is the axis they do not fill.

function buildPyramidGeometry() {
  // Square base ON the unit square: four radial segments started at 45° put the base
  // corners at (±0.5, ±0.5), which is sqrt(1/2) from the axis — hence that radius.
  return new THREE.ConeGeometry(Math.SQRT1_2, 1, 4, 1, false, Math.PI / 4);
}

function buildDomeGeometry() {
  // Top half of a sphere (phiLength π/2), stretched to fill the box in y and dropped so
  // its flat face IS the box floor — a dome sits on the ground at y = position - size/2
  // like every other shape, instead of hovering a half-height up.
  const geom = new THREE.SphereGeometry(0.5, 20, 8, 0, Math.PI * 2, 0, Math.PI / 2);
  geom.scale(1, 2, 1);
...
function buildCapsuleGeometry() {
  // A pill of radius 0.5 and body 1 is 2 tall, so it is halved in y to fit the unit
  // box. Consequence worth knowing when building with it: the caps are TRUE
  // hemispheres exactly when a part is twice as tall as it is wide (size [4,8,4]);
  // away from that ratio they are ellipsoidal, which is what makes a squat pill read
  // as a lozenge and a tall one as a rod.
  const geom = new THREE.CapsuleGeometry(0.5, 1, 8, 20);
  geom.scale(1, 0.5, 1);
  return geom;
}

function buildPrismGeometry() {
  // A hexagon inscribed in the unit circle reaches its full diameter across two of its
  // corners and only cos(30°) = 0.866 across its flats, so a raw one does not fill the
  // unit box. Stretched across the flats for the same reason the star is: `size` must
  // mean the same thing for every shape, and 13% of width missing on one axis is a gap
  // you see when you build a wall out of them.
  const geom = new THREE.CylinderGeometry(0.5, 0.5, 1, 6);
  geom.scale(1 / Math.cos(Math.PI / 6), 1, 1);
  return geom;
}

function buildTubeGeometry() {
  // A pipe: a CLOSED rectangular profile revolved around Y, so it has an outer wall, a
  // bore wall and two annular rims — a real hollow solid. Not an open-ended cylinder:
  // a single-sided wall disappears when you look into it, and this file cannot ask for
  // DoubleSide on one shape because materials are shared through getMaterial's cache.
  // Profile order (up the outside, in across the top, down the bore, out across the
  // bottom) is what makes LatheGeometry's normals point out of the solid on all four.
  const profile = [
...
// Ring and star are plates in the XY plane — the hole/points face ±z, so a hoop at
// spawn is something you look THROUGH, and `rotation: [90, 0, 0]` lays either one flat
// as a halo or a floor medallion. Their z extent is 0.3, not 1: depth is scaled by
// size[2] like everything else, so a ring's tube and a star's thickness are 30% of it.
const PLATE_DEPTH = 0.3;

function buildRingGeometry() {
  return new THREE.TorusGeometry(0.35, PLATE_DEPTH / 2, 12, 28);
}

function buildStarGeometry() {
  // Five-pointed star: 10 perimeter vertices alternating outer 0.5 / inner 0.19 (the
  // classic pentagram ratio, which is what stops the arms looking like a blunt cog),
  // a triangle fan per face and one quad per rim segment. Non-indexed with explicit
  // UVs, same as the wedge — a mapped material samples one texel without them.
  const POINTS = 5;
  const R_OUT = 0.5;
  const R_IN = 0.19;
  const hz = PLATE_DEPTH / 2;
  const rim = [];
  for (let i = 0; i < POINTS * 2; i++) {
    const r = i % 2 === 0 ? R_OUT : R_IN;
    // -90° start so a point aims at +y (a star stands up straight).
    const a = (i / (POINTS * 2)) * Math.PI * 2 - Math.PI / 2;
    rim.push([Math.cos(a) * r, Math.sin(a) * r]);
  }
  // Five points around a circle cannot be symmetric in a square: the raw star spans 0.95
  // of x, and in y it reaches −0.5 at the bottom point but only +0.40 between the two top
  // ones. It is therefore CENTRED and stretched onto its own bounding box, because `size`
  // has to mean the same thing for all thirteen shapes — a part that does not reach its
  // own edges leaves a gap when you build flush against it, and it would also hang off
  // centre inside its own collider. The stretch is 5% in x and 11% in y, which is not
  // something anyone can see on a star.
  const bounds = (axis) => {
    let lo = Infinity, hi = -Infinity;
    for (const p of rim) { if (p[axis] < lo) lo = p[axis]; if (p[axis] > hi) hi = p[axis]; }
    return { lo, hi, mid: (lo + hi) / 2, span: hi - lo };
  };
  const bx = bounds(0), by = bounds(1);
  for (const p of rim) {
    p[0] = (p[0] - bx.mid) / bx.span;
    p[1] = (p[1] - by.mid) / by.span;
  }

  const positions = [];
  const uvs = [];
  const uvOf = (x, y) => [x + 0.5, y + 0.5];
  const push = (x, y, z, u, v) => { positions.push(x, y, z); uvs.push(u, v); };
  const tri = (a, b, c) => {
    for (const p of [a, b, c]) {
      const [u, v] = uvOf(p[0], p[1]);
      push(p[0], p[1], p[2], u, v);
    }
  };
  ...
  for (let i = 0; i < rim.length; i++) {
    ...
    // front (+z, CCW seen from +z) and back (-z, reversed)
    tri([0, 0, hz], [p[0], p[1], hz], [q[0], q[1], hz]);
    tri([0, 0, -hz], [q[0], q[1], -hz], [p[0], p[1], -hz]);
    // rim quad
    tri([p[0], p[1], hz], [p[0], p[1], -hz], [q[0], q[1], -hz]);
    tri([p[0], p[1], hz], [q[0], q[1], -hz], [q[0], q[1], hz]);
  }
...
function buildUnitGeometry(shape) {
  switch (shape) {
    ...
    // spec 03 §5.2a, appended 2026-09-25 — order here is cosmetic, but the order of
    // the same nine names in place.js/pack.js/protocol.js is the wire format.
    case "cone": return new THREE.ConeGeometry(0.5, 1, 24);
    case "pyramid": return buildPyramidGeometry();
    case "prism": return buildPrismGeometry();
    case "dome": return buildDomeGeometry();
    case "capsule": return buildCapsuleGeometry();
    case "ring": return buildRingGeometry();
    case "star": return buildStarGeometry();
    case "diamond": return new THREE.OctahedronGeometry(0.5);
    case "tube": return buildTubeGeometry();
    default:
      throw new Error(`unknown shape '${shape}' for geometry cache`);
  }
}

// Exported since 2026-09-25 so src/platform/services/avatar/rig.js can build avatar prims
// from the SAME shapes engine parts use. Before that the two kept separate lists and a
// mismatch was silent: an item asking for a shape the rig did not know rendered as a plain
// box with nothing logged anywhere. The geometry returned is the CACHE's own, normalised to
// the unit box — a caller that scales it clones first (the rig does).
