...
function partDef(stage, p, i) {
  ...
  const color = map.diff ? pal.color : map.diffColor ? stage.color : map.color;
  const def = {
    id: partId(stage, p, i),
    // §5.14's scenery kinds carry their shape on the PART (one kind per shape would be
    // nine rows of the same thing); everything else takes its kind's shape.
    shape: p.shape || map.shape,
    size: p.size.slice(),
  ...
  // A lamp's colour is the part's colour, so a difficulty-tinted glow lights in its own
  // colour without the table having to say it twice (spec 03 §5.1b).
  if (map.light) def.light = { color, intensity: map.light.intensity, range: map.light.range };
  return def;
}
...
function payThrough(ctx, done) {
  ...
  for (let m = state.paid + 1; m <= done; m++) {
    ...
    total += REWARDS[row.diff] * (row.gauntlet ? 2 : 1);
