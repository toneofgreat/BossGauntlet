...
// The last roster row: its exit carries the winners' area instead of an onward hop.
...
function createCore(rng) {
  ...
  function pickRise(g) {
    // RISE_SAFETY: the cap is the exact envelope, and drawing right up to it puts hops ON
    // the boundary, where 08:R3's check fails by float dust ("gap 8.65 exceeds the jump
    // envelope 8.65 at rise 0.31" — stage 93, 2026-09-25). A rise is never worth 3
    // hundredths of a stud, so the picker stays inside the envelope by that much.
    const RISE_SAFETY = 0.03;
    const cap = Math.min(2, (FEAS.COMBO_MAX - g) / 1.3 - 0.15, FEAS.RISE_MAX, physicsRiseCap(g)) - RISE_SAFETY;
    let rise;
    if (rng() < 0.55 || cap <= 0.33) rise = rng() < 0.5 ? 0 : -rr(0.3, 1.5);
...
function createStages(core, seg) {
  ...
  // §5.8.1's TOWERS are gone (2026-09-25). Fourteen stages used to be a glass shell
  // with a helix of rings in it; they are ordinary `mixed` stages now, at the same stage
  // numbers and the same budgets, so this file no longer emits `towerShell`, `tower` or
  // `tower0`. The scenery pass below is what makes a stage worth looking at instead.

  // §5.14 — SCENERY (added 2026-09-25). Nothing here collides, nothing here counts
  // against a platform budget, and nothing here is on the route: strip the whole pass out
  // and every hop in the chart is the same hop. That is the contract that lets it be
  // generous — and it is what 08:R3 (every hop makeable) keeps honest, since a decor part
  // is never a node.
  // True when a decor part would sit in a path node's headroom. Measured exactly the way
  // 08:R4 measures it — the part's BOTTOM against the node's top, 0.01 < clear < 7 — and
  // against the same node set the rule uses: this stage's and its neighbours'. This is the
  // generator refusing to break the rule rather than being told off for it afterwards.
  //
  // `decorRoutes` is filled by the decor pass (see buildLayout: decor runs AFTER every
  // stage exists, because a stage's scenery can shadow the NEXT stage's nodes and those do
  // not exist while the stage itself is being generated).
  let decorRoutes = [];

  function overRoute(x, z, bottom, hx, hz) {
    for (const path of decorRoutes) {
      for (const n of path) {
        if (Math.abs(x - n.x) >= n.hx + hx || Math.abs(z - n.z) >= n.hz + hz) continue;
        const clear = bottom - n.top;
        if (clear > 0.01 && clear < 7) return true;
      }
    }
    return false;
  }

  // part(), but silently skipped when it would steal a node's headroom.
  function decorPart(kind, pos, size, extra) {
    const bottom = pos[1] - size[1] / 2;
    if (overRoute(pos[0], pos[2], bottom, size[0] / 2, size[2] / 2)) return null;
    return part(kind, pos, size, extra);
  }

  function emitDecor(row, routes) {
    decorRoutes = routes || [st.stage.path];
    const wasCount = st.count;
    st.count = false;
    const cp = st.stage.cp;
    const p = perp();
    ...
    // A lit lamp post either side of the checkpoint. The obby runs at y 100 under its own
    // sky; before lamps existed, a night preset made the chart unreadable.
    if (cp) {
      // Clear of the checkpoint's own footprint, whatever that is: stage 1's pad sits on
      // the plaza, whose node is 12 studs wide.
      const cpNode = st.stage.path.find((n) => n.hop === "cp") || { hx: 3, hz: 3 };
      const out = Math.max(7, Math.max(cpNode.hx, cpNode.hz) + 5);
      for (const side of [-1, 1]) {
        const lx = cp[0] + p.x * out * side;
        const lz = cp[2] + p.z * out * side;
        decorPart("lampPost", [lx, cp[1] + 3, lz], [0.6, 7, 0.6]);
        decorPart("lampGlow", [lx, cp[1] + 7.2, lz], [1.8, 1.8, 1.8]);
      }
    }

    // Three props along the stage, chosen by difficulty band so the chart CHANGES as it
    // gets harder: bushes, then crystals, then spikes, then galaxy orbs.
    const nodes = st.stage.path.filter((n) => n.hop !== "cp" && n.hop !== "conn");
    for (let i = 0; i < 3 && nodes.length > 4; i++) {
      const n = nodes[Math.floor(((i + 1) / 4) * nodes.length)];
      if (!n) continue;
      const side = i % 2 === 0 ? 1 : -1;
      const ox = n.x + p.x * (n.hx + 6) * side;
      const oz = n.z + p.z * (n.hz + 6) * side;
      if (band <= 7) {
        decorPart("prop", [ox, n.top - 1.4, oz], [1, 3, 1], { shape: "cylinder" });
        decorPart("prop", [ox, n.top + 1.6, oz], [4, 4, 4], { shape: "sphere" });
      } else if (band <= 13) {
        decorPart("prop", [ox, n.top - 0.6, oz], [3, 1.4, 3], { shape: "prism" });
        decorPart("propGlow", [ox, n.top + 2, oz], [2.2, 4, 2.2], { shape: "diamond", spin: 30 });
      } else if (band <= 18) {
        decorPart("prop", [ox, n.top - 0.5, oz], [3.4, 1, 3.4], { shape: "cylinder" });
        decorPart("prop", [ox, n.top + 2.5, oz], [2.6, 6, 2.6], { shape: "cone" });
        decorPart("propGlow", [ox, n.top + 6.4, oz], [1.6, 1.6, 1.6], { shape: "sphere" });
      } else {
        decorPart("propGlow", [ox, n.top + 3, oz], [4, 4, 4], { shape: "sphere" });
        decorPart("propGlow", [ox, n.top + 3, oz], [7, 7, 0.8], { shape: "ring", spin: 40 });
      }
    }

    // A gauntlet announces itself: two pillars, a lintel and a spinning star over the
    // checkpoint it starts from. It is the landmark the tower used to be.
    if (row.gauntlet && cp) {
      // The arch stands clear of the pad and its lintel is well above head height, so the
      // landmark never becomes a ceiling (08:R4 gives a node 7 studs; this gives 12).
      for (const side of [-1, 1]) {
        decorPart("prop", [cp[0] + p.x * 8 * side, cp[1] + 6, cp[2] + p.z * 8 * side], [2.4, 12, 2.4], { shape: "prism" });
      }
      const [lx, lz] = orient(2.6, 18.4);
      decorPart("prop", [cp[0], cp[1] + 13.4, cp[2]], [lx, 1.6, lz]);
      decorPart("propGlow", [cp[0], cp[1] + 17, cp[2]], [5, 5, 1], { shape: "star", spin: 45 });
    }

    st.count = wasCount;
  }

  return { placeCheckpoint, emitGate, emitSign, maybeConnector, emitSpecial, emitThemed, emitDecor, st };
}
...
// §5.8.3 — the reward for 93 stages: a gold terrace off the final stage's exit, a
...
const CORE_KINDS = new Set([
  "platform", "killTile", "deco", "wall", "gate", "sign",
  "spinnerHub", "spinnerBar", "cpPad", "winPad", "winFloor", "confettiBar", "confettiBarAlt",
  // §5.14 scenery (2026-09-25). `towerShell` left with the towers.
  "lampPost", "lampGlow", "prop", "propGlow",
]);
...
export function buildLayout() {
  ...
  for (let i = 0; i < ROSTER.length; i++) {
    ...
    api.emitSign();
    // §5.14's scenery runs in a SECOND pass (below), not here: a stage's props have to
    // keep clear of the next stage's path too, and that path does not exist yet.
    decorQueue.push({ row, stage, d, heading: st.heading });
    api.maybeConnector();
  }

  // §5.14 — the scenery pass, over the finished chart. Each stage's props are placed
  // against its own route and its neighbours', which is the node set 08:R4 judges them by.
  for (let i = 0; i < decorQueue.length; i++) {
    const entry = decorQueue[i];
    st.stage = entry.stage;
    st.d = entry.d;
    st.heading = entry.heading;
    const routes = [stages[i - 1], stages[i], stages[i + 1]]
      .filter(Boolean)
      .map((s) => s.path);
    api.emitDecor(entry.row, routes);
  }
