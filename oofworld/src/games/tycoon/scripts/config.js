...
export const LAYOUT = Object.freeze({
  ...
  // ---- the tower: floors 2 and 3 (spec 10 §5.13, added 2026-09-25) ----------------
  // `top` is the standing surface, `height` the clear height to the ceiling above it.
  // Floor 0 is §5.1's plot: its surface is y 0 and its "ceiling" is the Roof at 17.25,
  // which is exactly what floor 1 stands on — so the floors STACK with no gaps and no
  // number repeated in two places. Each floor's belt and bin are its own (§5.13): a drop
  // never travels between floors, which is why two more floors needed no change to the
  // drop lifecycle.
  // `beltX` is the axis a floor's belt, bin, spouts and drops all share. Floor 0's is the
  // middle of the plot, as §5.1 always had it; the upper floors run their belts down the
  // WEST side, which is what leaves floor 3's east half free for the office (§5.13) —
  // furniture at x 0 would otherwise be standing on a conveyor.
  FLOORS: Object.freeze([
    Object.freeze({ top: 0, height: 16.5, beltX: 0, beltZ: -2, beltLength: 68, binZ: -41 }),
    Object.freeze({ top: 17.25, height: 15, beltX: -18, beltZ: 18, beltLength: 52, binZ: -13 }),
    Object.freeze({ top: 33.75, height: 14, beltX: -18, beltZ: 18, beltLength: 52, binZ: -13 }),
  ]),
  // The tower footprint. Narrower and shorter than the plot on purpose: it leaves the
  // Boss Chopper's helipad (z −30) under open sky and the Roof Ramp (x 44) outside the
  // wall line, so the ramp still lands on open roof and you walk in through the east
  // doorway of floor 2's front wall.
  TOWER_HALF_X: 34,
  TOWER_Z_MIN: -18,
  TOWER_Z_MAX: 50,
  TOWER_STAIR_STEPS: 9,
  // The stairwell the ceiling slab leaves open for the stair to arrive through. It has to
  // cover where the TOP step lands: the stair starts at TOWER_Z_MIN + 6 and climbs +z on
  // 5-stud treads, 8 wide, centred on x -26.
  TOWER_STAIRWELL: Object.freeze({ xMin: -31, xMax: -21, zMin: 24, zMax: 38 }),
  // Each upper floor's collection region, same shape as COLLECT_MIN/MAX and for the same
  // reason (the conveyor's carry is positional, so the region has to overlap the belt's
  // last carried tick — see COLLECT_MIN's note).
  COLLECT_Y_SPAN: 6,
...
export const PURCHASES = Object.freeze([
  ...
  // ---- the tower (spec 10 §5.13, added 2026-09-25) --------------------------------
  // Read the `requires` column of the five droppers below: every one is null, exactly
  // like d01-d11. **No dropper is ever gated behind another purchase** — the only thing
  // between a player and a dropper is its price. A FLOOR is a building (floor3 wants
  // floor2 standing, which is structural, not a fee), and no floor requires a dropper.
  //
  // The cost ladder continues §5.3's ~1.5-3x steps: floor2 500K → d12 8M → d13 20M →
  // u3 30M → d14 50M → floor3 80M → d15 120M → d16 300M → throne 1B.
  Object.freeze({ id: "floor2", kind: "building", name: "Second Floor", cost: 500000,
    requires: "roof", pad: [34, 17.75, 44] }),
  Object.freeze({ id: "d12", kind: "dropper", name: "Emerald Dropper", cost: 8000000,
    requires: null, pad: [26, 17.75, 40], value: 60000, color: "#2fbf9b", dropZ: 40, side: -1, floor: 1 }),
  Object.freeze({ id: "d13", kind: "dropper", name: "Sapphire Dropper", cost: 20000000,
    requires: null, pad: [26, 17.75, 30], value: 150000, color: "#2a67c9", dropZ: 30, side: 1, floor: 1 }),
  Object.freeze({ id: "u3", kind: "upgrader", name: "Hyper Upgrader ×3", cost: 30000000,
    requires: null, pad: [26, 17.75, 20], mult: 3, archZ: 8, floor: 1 }),
  Object.freeze({ id: "d14", kind: "dropper", name: "Ruby Dropper", cost: 50000000,
    requires: null, pad: [26, 17.75, 10], value: 400000, color: "#d93025", dropZ: 20, side: -1, floor: 1 }),
  Object.freeze({ id: "floor3", kind: "building", name: "Third Floor", cost: 80000000,
    requires: "floor2", pad: [26, 17.75, 0], padFloor: 1 }),
  Object.freeze({ id: "d15", kind: "dropper", name: "Diamond Dropper", cost: 120000000,
    requires: null, pad: [26, 34.25, 40], value: 1000000, color: "#35e0e0", dropZ: 40, side: -1, floor: 2 }),
  Object.freeze({ id: "d16", kind: "dropper", name: "Void Dropper", cost: 300000000,
    requires: null, pad: [26, 34.25, 30], value: 2500000, color: "#6b3fa0", dropZ: 30, side: 1, floor: 2 }),
  Object.freeze({ id: "bossthrone", kind: "building", name: "The Boss Office", cost: 1000000000,
    requires: "floor3", pad: [26, 34.25, 20], padFloor: 2 }),
