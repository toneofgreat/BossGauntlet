...
import {
  buildFloorLine, buildFloorShell, buildOffice, buildGroundDetail, dropperParts,
} from "./tower.js";
...
export function buildStatic(ctx, state) {
  ...
  // §5.13's detail pass on the ground floor: rollers, a collector hood with floodlights,
  // floor markings, overhead pipes. Static, like the belt it dresses — it is not a
  // purchase, so it is here rather than in buildPurchase.
  buildGroundDetail((def) => track(ctx, state, "plot", def));

  // A pad only exists once there is a floor under it. Before floor 2 is bought, its
  // pads would hang in mid-air 17 studs up — visible, unreachable, and a lie about what
  // you can buy. Buying a floor builds its pads (see buildFloor's tail).
  for (const p of PURCHASES) if (padReachable(state, p)) buildPad(ctx, state, p);
...
// Which floor a pad stands on: its own `padFloor` if it has one, else the floor of the
// machine it buys, else the ground. A pad is reachable when that floor exists.
function padFloorOf(p) {
  if (p.padFloor !== undefined) return p.padFloor;
  return p.floor || 0;
}

const FLOOR_PURCHASE_ID = ["", "floor2", "floor3"];

function padReachable(state, p) {
  const floor = padFloorOf(p);
  if (floor === 0) return true;
  return !!state.save.purchased[FLOOR_PURCHASE_ID[floor]];
}
...
export function buildPurchase(ctx, state, id, opts = {}) {
  ...
  else if (id === "floor2") buildFloor(ctx, state, p, 1);
  else if (id === "floor3") buildFloor(ctx, state, p, 2);
  else if (id === "bossthrone") buildOffice(adder(ctx, state, p.id), labeller(ctx, state));
  else if (p.kind === "gear") buildGearPad(ctx, state, p);
  else if (id.startsWith("aura")) buildAura(ctx, state, p);
  repaintPad(ctx, state, p);
}

// A small adapter so tower.js can build parts and labels without knowing how this file
// tracks them for disposal.
function adder(ctx, state, id) {
  return (def) => track(ctx, state, id, def);
}
function labeller(ctx, state) {
  return (key, pos, lines, color, w, h) => addLabel(ctx, state, key, pos, lines, color, w, h);
}

// The dropper machine moved to tower.js with the §5.13 detail pass (a pillar, a plinth,
// a body, a vent, a prism cap, an arm, a pipe, a lit spout and a rim), and it reads
// `p.floor` so the same builder serves all three floors.
function buildDropper(ctx, state, p) {
  dropperParts(adder(ctx, state, p.id), labeller(ctx, state), p);
}

// One floor of the tower: its shell (walls, windows, ceiling, pillars, lamps, and — on
// floor 2 — the stair up to floor 3) and its own production line.
function buildFloor(ctx, state, p, floorIndex) {
  const add = adder(ctx, state, p.id);
  buildFloorShell(add, floorIndex, {
    stairUp: floorIndex === 1,
    wallColor: floorIndex === 1 ? "#b4b4b4" : "#d8d3c8",
    trimColor: floorIndex === 1 ? "#8d8d94" : "#a89b82",
  });
  buildFloorLine(add, labeller(ctx, state), floorIndex);
  // The pads that stand on this floor can exist now.
  for (const q of PURCHASES) {
    if (padFloorOf(q) === floorIndex && !state.padsById[q.id]) {
      if (state.save.purchased[q.id]) buildPadOwned(ctx, state, q);
      else buildPad(ctx, state, q);
    }
  }
  refreshPadLabels(ctx, state);
}
...
function buildArch(ctx, state, p) {
  const L = LAYOUT;
  const y = LAYOUT.FLOORS[p.floor || 0].top;
  const extra = { material: "neon", transparency: L.ARCH_TRANSPARENCY, canCollide: false };
  for (const sx of [-1, 1]) {
    track(ctx, state, p.id, box(L.ARCH_PILLAR_SIZE, [sx * L.ARCH_PILLAR_X, y + 3, p.archZ], L.ARCH_COLOR, extra));
    // §5.13 detail: a real frame either side of the glowing field, and a lamp in it.
    track(ctx, state, p.id, box([2, 0.8, 2], [sx * L.ARCH_PILLAR_X, y + 0.4, p.archZ], "#5c6478", { material: "metal" }));
  }
  track(ctx, state, p.id, box(L.ARCH_BEAM_SIZE, [0, y + L.ARCH_BEAM_Y, p.archZ], L.ARCH_COLOR, extra));
  track(ctx, state, p.id, box([11, 1, 2.4], [0, y + L.ARCH_BEAM_Y + 1.4, p.archZ], "#5c6478", {
    material: "metal", light: { color: L.ARCH_COLOR, intensity: 1.6, range: 20 },
  }));
  addLabel(ctx, state, "machine:" + p.id, [0, y + L.ARCH_BEAM_Y + 3, p.archZ], ["×3 UPGRADER"], L.ARCH_COLOR, 8, 2);
}
...
// A drop is collected inside ANY floor's bin region (§5.13). Floor 0's is §5.1's
// verbatim; the upper floors' are derived from their own belt and bin, with the same
// +z overlap onto the belt's last carried tick that COLLECT_MAX explains.
function collectRegions() {
  const out = [{ lo: LAYOUT.COLLECT_MIN, hi: LAYOUT.COLLECT_MAX }];
  for (let i = 1; i < LAYOUT.FLOORS.length; i++) {
    const f = LAYOUT.FLOORS[i];
    const beltEnd = f.beltZ - f.beltLength / 2; // the belt's −z end
    out.push({
      lo: [f.beltX - 5, f.top, f.binZ - 4],
      hi: [f.beltX + 5, f.top + LAYOUT.COLLECT_Y_SPAN, beltEnd + 0.5],
    });
  }
  return out;
}

const COLLECT_REGIONS = collectRegions();

function inCollector(pos) {
  for (const r of COLLECT_REGIONS) {
    if (pos[0] >= r.lo[0] && pos[0] <= r.hi[0] &&
        pos[1] >= r.lo[1] && pos[1] <= r.hi[1] &&
        pos[2] >= r.lo[2] && pos[2] <= r.hi[2]) return true;
  }
  return false;
}
...
export function spawnDrop(ctx, state, id) {
  ...
  const partId = ctx.engine.parts.create({
    // Spawn height is the dropper's OWN floor plus §6's DROP_SPAWN_Y, so an upper-floor
    // drop falls the same 6 studs onto the same kind of belt (§5.13).
    shape: "box", size: [s, s, s],
    position: [
      LAYOUT.FLOORS[p.floor || 0].beltX,
      LAYOUT.FLOORS[p.floor || 0].top + TUNING.DROP_SPAWN_Y,
      p.dropZ,
    ],
