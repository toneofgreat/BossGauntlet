...
export function importSaveCode(code) {
  ...
  if (version !== "1") return { ok: false, error: "This code is from a newer version of OofWorld." };
  ...
  for (const domainName of ["profile", "economy", "avatar", "badges"]) {
    if (tooNew(domainName, parsed[domainName])) {
      return { ok: false, error: "This code is from a newer version of OofWorld." };
    }
  }
  for (const obj of Object.values(parsed.places || {})) {
    if (tooNew("place", obj)) return { ok: false, error: "This code is from a newer version of OofWorld." };
  }
