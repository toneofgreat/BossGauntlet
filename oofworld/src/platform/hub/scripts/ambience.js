...
// ctx.engine.renderer.applyLighting seam". **That seam exists since 2026-09-25** (spec
// 04 §5.7), so this is now the path every Hub load takes, and the two gaps called out in
// applyToScene are gone with it: the sun DIRECTION is honoured, and the fog survives a
// later quality-tier change because the renderer re-reads the config it was given.
//
// applyToScene below is kept as the fallback for a host that hands a Place a ctx without
// the seam (a test rig, or an older embedding). It is no longer the path that runs.
