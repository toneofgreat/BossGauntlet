...
// The Brick Climb's gate arch stands at z 62 (brickclimb.js's BASE.z - 14); its labels
// ride just south of the lintel so they read from the plaza.
const CLIMB_SIGN_Z = 63.7;
...
export function buildSigns(ctx) {
  ...
  // The Brick Climb's gate and its prize, at the south end of the plaza (spec 06
  // §5.3.7). Both face +z, the way a label mesh already faces, because the player
  // arrives from the fountain — i.e. from the north.
  track.label(placeLabel(ctx, {
    text: "BRICK CLIMB", worldW: 15, worldH: 2.6, color: PLAQUE_GOLD, font: LABEL_FONT,
    position: [0, 17.6, CLIMB_SIGN_Z],
  }));
  track.label(placeLabel(ctx, {
    text: "→ BRICK LEGEND", worldW: 13, worldH: 1.8, color: SIGN_TEXT, font: LABEL_FONT,
    position: [0, 13.4, CLIMB_SIGN_Z],
  }));
