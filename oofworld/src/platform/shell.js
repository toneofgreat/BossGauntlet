...
// installed under all four sibling names (spec 12 §3.7 binding reconciliation) — the
// last one is the pre-rename alias, kept because the harness reads it (ARCHITECTURE §4).
window.__oof = window.__oofDebug = window.__oofworld = window.__oofcubes = debugHandle;
...
// openExternal(url, opts) -> Promise<boolean> — spec 06 §5.6.5b. The one door out of
// OofWorld, and the reason it is here rather than in a Place: the player is told where
// they are going and chooses, in a platform dialog, before any tab opens.
//
// The URL is resolved against the page's own location, so a relative "../x.html" reaches
// the sibling game on whatever host this build is served from and an off-site absolute
// URL is refused outright — a Place cannot use this to send anyone anywhere it likes.
// `noopener` is not optional: the new tab must not get a handle on this window.
function openExternal(url, opts = {}) {
  let resolved;
  try {
    resolved = new URL(String(url), location.href);
  } catch (err) {
    console.warn("[oof] openExternal: not a URL", url, err);
    return Promise.resolve(false);
  }
  if (resolved.origin !== location.origin) {
    console.warn("[oof] openExternal refused an off-site URL:", resolved.href);
    return Promise.resolve(false);
  }
  return dialog({
    title: opts.title || "Leaving OofWorld",
    body: opts.body || `This opens ${resolved.pathname} in a new tab.`,
    buttons: [
      { id: "cancel", label: "Stay here", variant: "secondary" },
      { id: "go", label: opts.confirmLabel || "Open it", variant: "primary" },
    ],
  }).then((id) => {
    if (id !== "go") return false;
    const win = window.open(resolved.href, "_blank", "noopener");
    if (!win) {
      // A blocked popup is the common case on a phone, and silence here reads as a
      // broken button — so say what happened and leave the door findable.
      ui.toast({ variant: "error", title: "Your browser blocked the new tab" });
      return false;
    }
    return true;
  });
...
const ui = {
  ...
  // openExternal(url, { title, body, confirmLabel }) — spec 06 §5.6.5b, added 2026-09-25
  // for the Hub's Brick Legend door. A Place never opens a tab by itself: leaving the
  // site is a decision the player makes in a platform dialog, in the platform's own
  // voice, and the URL is resolved against the page so a Place cannot name a host. It
  // resolves true if the tab was opened, false if the player said no.
  openExternal: (url, opts) => safely(() => openExternal(url, opts), Promise.resolve(false)),
...
function buildCtx(emitter, slug) {
  return {
    engine: {
      ...
      // The renderer seam (spec 04 §5.7, added 2026-09-25). Two methods, both of which a
      // Place already had a worse way of reaching: applyLighting is how a Place changes
      // its own sky/sun/fog after load — the Hub's ambience presets used to write the
      // scene's light objects by hand, which could not honour a sun DIRECTION at all
      // (the renderer re-derives the sun's position from its own stored vector every
      // frame) and lost its fog the moment the quality tier changed. `lights` is
      // read-only reporting for a Place that wants to say how many of its lamps are lit.
      renderer: {
        applyLighting: (cfg) => renderer.applyLighting(cfg),
        lights: {
          poolSize: () => renderer.lights.poolSize(),
          activeCount: () => renderer.lights.activeCount(),
        },
      },
      physics: {
      ...
      parts: {
        ...
        // spec 03 §5.1b: make a part a lamp, retune it, or (with null) stop it being
        // one. The Hub's lamp posts use it to burn brighter at night.
        setLight: (id, light) => parts.setLight(id, light),
...
async function loadPlaceInto(entry, slug) {
  ...
  // `lights` is the renderer's point-light pool seam (spec 02 §5.2a): parts.js pushes a
  // provider into it so lamp parts can borrow the pool. Wired here because the shell is
  // the only place that holds both modules.
  parts.init({ scene: renderer.scene, physics: physicsForParts, audio, events, lights: renderer.lights });
