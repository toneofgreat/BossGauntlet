...
const SHAPES = [
  ...
  // 2026-09-25 shape pack (spec 03 §5.2a) — APPEND-ONLY, this is the wire format.
  "cone", "pyramid", "prism", "dome", "capsule", "ring", "star", "diamond", "tube",
];
const MATERIALS = [
  "plastic", "neon", "metal", "grass", "lava", "ice", "glass", "wood",
  // 2026-09-25 material pack (spec 03 §5.1a) — APPEND-ONLY, same reason.
  "brick", "marble", "sand", "concrete", "gold", "cobble", "snow", "galaxy",
];
// spec 02 §5.5.10 TRACKS ids — APPEND-ONLY like the lists above (the index is the wire
// format). The last three joined on 2026-09-25, when Studio's world panel started
// offering every engine track instead of the first four.
const MUSIC_IDS = ["plaza", "ascent", "pump", "cashflow", "chill", "clash", "voidchill"];
...
const LIGHT_Q = 20; // lamp intensity and range, also to 0.05
...
function packPart(part, i, colorIdxFor, rowOfId) {
  ...
  // Elements 14 and 15 are both optional and POSITIONAL, so a part with a lamp and no
  // behaviors still pushes a placeholder 0 at 14 (added 2026-09-25 with the lamp block,
  // spec 11 §3.4). A 14-element row — every code published before this — is unchanged
  // and still unpacks, which is the whole reason the light went on the end.
  const behaviors = Array.isArray(part.behaviors) ? part.behaviors : [];
  const light = isPlainObject(part.light) ? part.light : null;
  if (behaviors.length > 0 || light) {
    row.push(behaviors.length > 0 ? behaviors.map((b) => packBehavior(b, i, rowOfId)) : 0);
  }
  if (light) {
    const intensity = light.intensity === undefined ? 1.2 : light.intensity;
    const range = light.range === undefined ? 24 : light.range;
    row.push([
      colorIdxFor(light.color || (part.color === undefined ? "#a3a2a5" : part.color), i),
      quantize(intensity, LIGHT_Q, "parts[" + i + "].light.intensity"),
      quantize(range, LIGHT_Q, "parts[" + i + "].light.range"),
    ]);
  }
  return row;
}
...
function unpackPart(row, i, palette, rowCount) {
  if (!Array.isArray(row) || row.length < 14 || row.length > 16) {
    throw unpackErr("part row " + i + " must have 14, 15 or 16 elements");
  }
  ...
  if (row.length === 16) {
    const packedLight = row[15];
    if (!Array.isArray(packedLight) || packedLight.length !== 3) {
      throw unpackErr("part row " + i + " light must be [colorIdx, intensity, range]");
    }
    const lightColor = palette[packedLight[0]];
    if (!lightColor) {
      throw unpackErr("part row " + i + " light points at colour index " + packedLight[0] + ", not in the palette");
    }
    part.light = {
      color: lightColor, // palette entries already carry the leading hash
      intensity: dequantize(packedLight[1], LIGHT_Q, "part " + i + " light.intensity"),
      range: dequantize(packedLight[2], LIGHT_Q, "part " + i + " light.range"),
    };
  }
  return part;
}
