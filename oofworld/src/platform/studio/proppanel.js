...
const MATERIALS = [
  "plastic", "neon", "metal", "grass", "lava", "ice", "glass", "wood",
  "brick", "marble", "sand", "concrete", "gold", "cobble", "snow", "galaxy", // 2026-09-25
];
const FLAT_THUMBS = { neon: "#7ec8ff", glass: "#cfe4ff" };

// All thirteen shapes (spec 03 §5.2a), so a part's shape can be CHANGED after it is
// placed instead of deleted and rebuilt — the panel already had the harder half of this
// (editor.writePartFields has always rebuilt the mesh on a shape change).
const SHAPE_GLYPHS = [
  ["box", "▦"], ["wedge", "◺"], ["cylinder", "⬤"], ["sphere", "●"],
  ["cone", "▲"], ["pyramid", "🔺"], ["prism", "⬢"], ["dome", "◓"],
  ["capsule", "💊"], ["ring", "◯"], ["star", "★"], ["diamond", "◆"], ["tube", "◎"],
];
const LAMP_MAX_INTENSITY = 8; // spec 04 §3.1 light.intensity ceiling
const LAMP_MAX_RANGE = 200; // spec 04 §3.1 light.range ceiling
const LAMP_DEFAULT = Object.freeze({ intensity: 1.2, range: 24 });
...
export function createPropPanel(container, opts = {}) {
  ...
  function shapeSection(part) {
    const wrap = el("div", "");
    wrap.appendChild(el("div", SECTION_LABEL, "SHAPE"));
    const grid = el("div", "display:grid;grid-template-columns:repeat(5,1fr);gap:4px;");
    for (const [name, glyph] of SHAPE_GLYPHS) {
      const cell = el("button", BTN + "height:34px;padding:0;font-size:17px;border-color:"
        + (name === part.shape ? ACCENT : LINE) + ";", glyph);
      cell.title = name;
      cell.setAttribute("aria-label", name);
      cell.addEventListener("click", () => {
        if (name === part.shape) return;
        // A sphere with three different extents and a cylinder with x !== z are both
        // rejected by spec 04 §3.4, so switching INTO one has to fix the size in the
        // same gesture or the creation stops being exportable. constrainSize is the
        // same helper the typed size fields use.
        const fixed = constrainSize(name, part.size, 0);
        const changed = fixed.some((v, i) => v !== part.size[i]);
        if (changed) ed.applyTransform(ed.selection(), ed.selection().map(() => ({ size: fixed })));
        commitProp("shape", name);
      });
      grid.appendChild(cell);
    }
    wrap.appendChild(grid);
  ...
  // ---- LAMP (spec 04 §3.1 `light`) --------------------------------------------------
  // A part with a lamp borrows one of the renderer's pooled point lights (spec 02
  // §5.2a). The panel says out loud that the pool is finite and nearest-first, because
  // "I placed 30 lamps and only 8 are lit" is otherwise a bug report.
  function lampSection(part) {
    const light = part.light || null;
    const wrap = el("div", "margin-top:10px;border-top:1px solid " + LINE + ";padding-top:8px;");
    const head = el("div", "display:flex;align-items:center;justify-content:space-between;");
    head.appendChild(el("div", "font-size:13px;color:" + TEXT + ";", "Lamp"));
    head.appendChild(toggleSwitch(!!light, (on) => {
      commitProp("light", on
        ? { color: part.color, intensity: LAMP_DEFAULT.intensity, range: LAMP_DEFAULT.range }
        : undefined);
    }));
    wrap.appendChild(head);
    if (!light) return wrap;

    const patch = (fields) => commitProp("light", { ...light, ...fields });

    const slider = (label, key, min, max, step, value) => {
      const row = el("div", "display:flex;gap:8px;align-items:center;margin-top:6px;");
      row.appendChild(el("div", "font-size:11px;color:" + MUTED + ";width:58px;", label));
      ...
      input.value = String(value);
      const readout = el("div", "font-size:12px;color:" + MUTED + ";width:34px;text-align:right;",
        String(value));
    ...
    wrap.appendChild(slider("Bright", "intensity", 0, LAMP_MAX_INTENSITY, 0.1, light.intensity));
    wrap.appendChild(slider("Reach", "range", 1, LAMP_MAX_RANGE, 1, light.range));

    const colorRow = el("div", "display:grid;grid-template-columns:repeat(8,1fr);gap:3px;margin-top:6px;");
    for (const hex of STUDIO_COLORS) {
      const swatch = el("button", "height:18px;border-radius:4px;cursor:pointer;background:" + hex + ";"
        + "border:2px solid " + (hex === light.color ? TEXT : "transparent") + ";");
      swatch.title = "Lamp colour " + hex;
      swatch.addEventListener("click", () => patch({ color: hex }));
      colorRow.appendChild(swatch);
    }
    wrap.appendChild(colorRow);
    wrap.appendChild(el("div", "font-size:10px;color:" + MUTED + ";margin-top:6px;",
      "The nearest lamps light up — 8 at a time on a fast machine, 2 on a phone."));
    return wrap;
  }
  ...
  function currentSignature() {
    ...
    return JSON.stringify([
      ...
      // Shape and lamp are part of the signature because both change which CONTROLS the
      // panel shows (a lamp's three rows appear and disappear), not just their values.
      parts.map((p) => p.shape + (p.light ? "L" + p.light.color + p.light.intensity + p.light.range : "")),
      [...expanded],
    ]);
  }
  ...
  function render(force) {
    ...
    if (parts.length === 1) root.appendChild(shapeSection(first));
    ...
    if (parts.length === 1) root.appendChild(lampSection(first));
