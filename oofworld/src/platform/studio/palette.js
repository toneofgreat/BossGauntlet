...
// The four shapes that get a button of their own, unchanged since §5.6.2 named them.
const SHAPES = [
...
// The 2026-09-25 shape pack (spec 03 §5.2a) lives behind ONE more button that opens a
// grid, rather than nine more buttons: the palette is a 76 px column on a desktop and a
// 64 px strip on a phone, and thirteen 60 px buttons do not fit either. Each carries the
// collider it actually gets (spec 03 §5.4a) in its note — a cone you can stand on the
// air above is worth knowing before you build a spike pit out of them.
const EXTRA_SHAPES = [
  { shape: "cone", glyph: "▲", label: "Cone", note: "collides as a cylinder" },
  { shape: "pyramid", glyph: "🔺", label: "Pyramid", note: "collides as a box" },
  { shape: "prism", glyph: "⬢", label: "Hex prism", note: "collides as a cylinder" },
  { shape: "dome", glyph: "◓", label: "Dome", note: "flat face on the floor" },
  { shape: "capsule", glyph: "💊", label: "Pill", note: "round caps at 1:2" },
  { shape: "ring", glyph: "◯", label: "Ring", note: "upright hoop, no collision" },
  { shape: "star", glyph: "★", label: "Star", note: "upright plate, no collision" },
  { shape: "diamond", glyph: "◆", label: "Diamond", note: "collides as a box" },
  { shape: "tube", glyph: "◎", label: "Pipe", note: "the bore is solid" },
];
const EXTRA_BY_SHAPE = new Map(EXTRA_SHAPES.map((e) => [e.shape, e]));

const MATERIALS = [
  "plastic", "neon", "metal", "grass", "lava", "ice", "glass", "wood",
  // 2026-09-25 material pack (spec 03 §5.1a). The popover grid is 4 wide, so sixteen
  // materials fill four rows of it exactly.
  "brick", "marble", "sand", "concrete", "gold", "cobble", "snow", "galaxy",
];

// §6's "default part sizes" row. Sphere is uniform because spec 04 rejects a
// non-uniform sphere outright, and cylinder x/z must match for the same reason — which
// is also why every round shape in the pack below is authored with x === z.
const DEFAULT_SIZES = Object.freeze({
  box: [4, 1, 4], wedge: [4, 2, 4], cylinder: [4, 1, 4], sphere: [2, 2, 2],
  cone: [4, 6, 4], pyramid: [6, 6, 6], prism: [4, 4, 4], dome: [6, 3, 6],
  capsule: [3, 6, 3], ring: [6, 6, 1], star: [5, 5, 1], diamond: [3, 4, 3],
  tube: [4, 6, 4],
});
...
export function createPalette(container, opts = {}) {
  ...
  // ---- "more shapes": one button, a grid of the nine in the pack -------------------
  // It shows the glyph of whichever pack shape is armed, so the palette still tells you
  // at a glance what the next tap will place.
  const moreBtn = el("button", squareButton(60, "font-size:24px;"), "◈");
  moreBtn.title = "More shapes";
  moreBtn.setAttribute("aria-label", "More shapes");
  moreBtn.addEventListener("click", () => togglePopover("shape"));
  ...
  function togglePopover(which) {
    ...
    closePopover();
    if (wasOpen) return;
    if (which === "material") popover = buildMaterialPopover();
    else if (which === "shape") popover = buildShapePopover();
    else popover = buildColorPopover();
    popover.dataset.which = which;
  }

  function buildShapePopover() {
    const node = popoverShell();
    const grid = el("div", "display:grid;grid-template-columns:repeat(3,72px);gap:8px;");
    for (const entry of EXTRA_SHAPES) {
      const cell = el("button", squareButton(72, "flex-direction:column;gap:2px;font-size:22px;"));
      cell.appendChild(el("div", "line-height:1;", entry.glyph));
      cell.appendChild(el("div", "font-size:9px;color:" + MUTED + ";", entry.label));
      cell.title = entry.label + " — " + entry.note;
  ...
  function setArmed(shape) {
    ...
    const extra = armed ? EXTRA_BY_SHAPE.get(armed) : null;
    moreBtn.textContent = extra ? extra.glyph : "◈";
    moreBtn.style.background = extra ? ACCENT : PANEL2;
    moreBtn.style.color = extra ? ON_ACCENT : TEXT;
    moreBtn.setAttribute("aria-pressed", extra ? "true" : "false");
    moreBtn.title = extra ? extra.label + " — " + extra.note : "More shapes";
  }
