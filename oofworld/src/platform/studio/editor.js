...
// Shapes that are drawn as flat plates inside their box (spec 03 §5.2a) — see the
// canCollide line in newPlacement for why they are placed non-colliding.
const PLATE_SHAPES = new Set(["ring", "star"]);
...
export function createEditor(doc, deps) {
  ...
  const ed = {
    ...
    writePartFields(id, fields) {
      ...
      // The lamp block (spec 04 §3.1) is tested with `in`, not against undefined:
      // turning a lamp OFF has to DELETE the key, because spec 04 rejects `light: null`
      // outright, and the undo of "turn it on" arrives here as an explicit undefined.
      if ("light" in fields) {
        if (fields.light) part.light = { ...fields.light };
        else delete part.light;
        rebuild = true; // parts.js registers a lamp when it builds the part
      }
  ...
  function ghostDefAt(clientX, clientY) {
    ...
    return {
      ...
      // The two PLATE shapes of the 2026-09-25 pack are placed non-colliding: a ring's
      // and a star's collider is the box AROUND them (spec 03 §5.4a), so a solid one is
      // a hoop you cannot jump through and points you bump into. Everything else is
      // solid, and the prop panel's Collide toggle switches either way.
      canCollide: !PLATE_SHAPES.has(armedShape),
      behaviors: [],
    };
  }
