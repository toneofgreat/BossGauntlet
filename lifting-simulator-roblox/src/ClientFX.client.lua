...
--------------------------------------------------------------------------------
-- Click -> lift: fire the Lift remote whenever a held lifting Tool is activated.
-- (Tool.Activated covers mouse click AND mobile tap while the tool is out.)
--------------------------------------------------------------------------------

local LiftEvent = Remotes:WaitForChild("Lift")

local hookedTools = setmetatable({}, { __mode = "k" }) -- weak: dead tools drop out
...
-- Tools live in the Backpack and move into the Character while equipped.
-- Both containers are recreated on respawn, so hook every new one.
local function onCharacterForLift(char)
	hookContainer(char)
...
localPlayer.CharacterAdded:Connect(onCharacterForLift)
if localPlayer.Character then onCharacterForLift(localPlayer.Character) end
